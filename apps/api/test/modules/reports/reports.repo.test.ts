...
describe('ReportsRepo', () => {
  ...
  describe('getPipelineByStage', () => {
    it('groups open deals by stage and sums value', async () => {
      const [org] = await testDb.db.insert(schema.organizations).values({ name: 'Co', slug: slug(), defaultCurrency: 'USD' }).returning();
      const [pl] = await testDb.db.insert(schema.pipelines).values({ organizationId: org!.id, name: 'Sales' }).returning();
      const [s1] = await testDb.db.insert(schema.pipelineStages).values({ organizationId: org!.id, pipelineId: pl!.id, name: 'Lead', orderIndex: 0 }).returning();
      const [s2] = await testDb.db.insert(schema.pipelineStages).values({ organizationId: org!.id, pipelineId: pl!.id, name: 'Negotiation', orderIndex: 1 }).returning();
    ...
    it('omits stages with no open deals', async () => {
      const [org] = await testDb.db.insert(schema.organizations).values({ name: 'Co', slug: slug(), defaultCurrency: 'USD' }).returning();
      const [pl] = await testDb.db.insert(schema.pipelines).values({ organizationId: org!.id, name: 'P' }).returning();
      await testDb.db.insert(schema.pipelineStages).values({ organizationId: org!.id, pipelineId: pl!.id, name: 'Empty', orderIndex: 0 });
