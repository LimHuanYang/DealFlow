...
describe('DealsRepo', () => {
  ...
  it('create assigns positionInStage = last + 1 within the target stage', async () => {
    const d1 = await repo.create(orgId, { name: 'Deal A', pipelineId, stageId: leadStageId });
    const d2 = await repo.create(orgId, { name: 'Deal B', pipelineId, stageId: leadStageId });
